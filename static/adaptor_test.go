...
var _ = Describe("func FromPackages() (handler adaptor detection)", func() {
	When("the the adaptor is created by a function call", func() {
		It("builds the configuration from the adapted type", func() {
			cfg := packages.Config{
				Mode: packages.LoadAllSyntax,
				Dir:  "testdata/handlers/adaptor-func",
			}
			...
			Expect(aggregate.TypeName()).To(
				Equal(
					"github.com/dogmatiq/configkit/static/testdata/handlers/adaptor-func.AggregateHandler",
				),
			)
			...
			Expect(process.TypeName()).To(
				Equal(
					"github.com/dogmatiq/configkit/static/testdata/handlers/adaptor-func.ProcessHandler",
				),
			)
			...
			Expect(projection.TypeName()).To(
				Equal(
					"github.com/dogmatiq/configkit/static/testdata/handlers/adaptor-func.ProjectionHandler",
				),
			)
			...
			Expect(integration.TypeName()).To(
				Equal(
					"github.com/dogmatiq/configkit/static/testdata/handlers/adaptor-func.IntegrationHandler",
				),
			)
